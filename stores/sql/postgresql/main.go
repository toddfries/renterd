...
func (tx *MainDatabaseTx) CompleteMultipartUpload(ctx context.Context, bucket, key, uploadID string, parts []api.MultipartCompletedPart, opts api.CompleteMultipartOptions) (string, error) {
	...
	updateSlicesStmt, err := tx.Prepare(ctx, `
		...
		FROM multipart_parts mpp
		WHERE s.db_multipart_part_id = mpp.id
		AND mpp.id = $3
	`)
	if err != nil {
...
func (tx *MainDatabaseTx) CreateBucket(ctx context.Context, bucket string, bp api.BucketPolicy) error {
	...
	res, err := tx.Exec(ctx, `
		INSERT INTO buckets (created_at, name, policy)
		VALUES ($1, $2, $3)
	...
	args = append(args, limit)
        res, err := tx.Exec(ctx, `
                ...
                WHERE id IN (
                        ...
                        WHERE c.fcid = ANY($1::text[]) AND slabs.health_valid_until >= $2
                        LIMIT $3
                )
        `, pq.Array(fcids), args[1], args[2])
	if err != nil {
		return 0, err
	}
...
func (tx *MainDatabaseTx) MakeDirsForPath(ctx context.Context, path string) (int64, error) {
	...
	if _, err := tx.Exec(ctx, `
		INSERT INTO directories (id, name, db_parent_id)
		VALUES ($1, '/', NULL)
	...
	insertDirStmt, err := tx.Prepare(ctx, `
		INSERT INTO directories (name, db_parent_id)
		VALUES ($1, $2)
		ON CONFLICT (name) DO UPDATE
		SET id = directories.id
	`)
	if err != nil {
